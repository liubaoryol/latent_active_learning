...
class TeamBoxWorldEnv(gym.Env):
    ...
    def step(self, action):
        ...
        if self.at_absorb_state:
            ...
            if isinstance(direction, str):
                ...
                agent_location = self.occupied_grids[0]
                agent_location, rew = self.move_agent(agent_location, direction)
                reward += rew
                self.occupied_grids[0] = agent_location
    ...
    def move_robot(self):
        ...
        # do the fastest policy: first up or down, then left
        diffx, diffy = robot_xy_target -self._robot_position 
        if diffx != 0:
            self._robot_position += np.array([math.copysign(1, diffx), 0], dtype=int)
        elif diffy != 0:
            self._robot_position += np.array([0, math.copysign(1, diffy)], dtype=int)
        else:
            if np.random.rand() < self.robot_pickup_param and self._robot_goal not in self._visited_goals:
